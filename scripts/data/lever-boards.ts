 ...
 *
 * Nearly empty, and not for lack of trying. `pnpm discover --source lever`
 ...
 * the harvester to the EU host with it, is the follow-up that would fill this
 * list; until then the few Slugs here were found by hand.
 *
 * Each Slug is probed against the live Lever API before it goes in, and a
 * Board that later dies is disabled rather than removed — the same rules
 * `greenhouse-boards.ts` spells out. `pnpm discover --source lever` prints a
 * paste-ready list; `pnpm seed:boards` writes it.
 */
